...
function UsersPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      const matchesSearch =
        ...
        getRoleLabel(user?.role).toLowerCase().includes(value);
  ...
  const statistics = useMemo(() => {
    return {
      ...
      registration: users.filter(
        (user) => user?.role === ROLES.REGISTRATION_OFFICER
      ).length,
      verification: users.filter(
        (user) => user?.role === ROLES.VERIFICATION_OFFICER
      ).length,
      viewers: users.filter((user) => user?.role === ROLES.VIEWER).length,
    };
  }, [users]);
  ...
  */
  ...
  const openEditForm = (user) => {
    ...
    setForm({
      ...
      password: "",
      role: user?.role || ROLES.REGISTRATION_OFFICER,
  ...
  */
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const returnedUser = extractUser(payload);

      if (editingUser) {
        setUsers((previous) =>
          previous.map((user) =>
            getUserId(user) === getUserId(editingUser)
              ? returnedUser || { ...user, ...body }
              : user
          )
        );
        ...
        if (returnedUser) {
          setUsers((previous) => [returnedUser, ...previous]);
        } else {
          await loadUsers();
        }
      ...
      showError(err.message || "Unable to save user account.");
    } finally {
      setSaving(false);
    }
  };
  ...
  */
  ...
  const updateStatus = async (user, nextStatus) => {
    ...
    if (userId === currentUserId && nextStatus === "suspended") {
      showError("You cannot suspend your own Super Admin account.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || "Unable to update user status."
        );
      }
      ...
      showError(err.message || "Unable to update user status.");
    } finally {
      setActionLoading("");
    }
  };

  const handleSuspend = (user) => updateStatus(user, "suspended");
  const handleActivate = (user) => updateStatus(user, "active");
  ...
  */
  ...
  const handlePasswordChange = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || "Unable to change user password."
        );
      }

      showSuccess("User password changed successfully.");
      closePasswordModal();
    } catch (err) {
      console.error("CHANGE USER PASSWORD ERROR:", err);

      showError(err.message || "Unable to change user password.");
    } finally {
      setSaving(false);
    }
  };
  ...
  */
  ...
  const openDeleteModal = (user) => {
    if (getUserId(user) === currentUserId) {
  ...
  */
  ...
  const handlePhotoSelect = (event) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      showError("Photo must not exceed 5MB.");
      event.target.value = "";
      return;
    }
  ...
  const uploadPhoto = async () => {
    if (!selectedUser || !photoFile) {
      showError("Please select a new photo first.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || "Unable to upload user photo."
        );
      }
      ...
      showSuccess("User photo uploaded successfully.");
      closePhotoModal();
    } catch (err) {
      console.error("USER PHOTO UPLOAD ERROR:", err);

      showError(err.message || "Unable to upload user photo.");
    } finally {
      setPhotoLoading("");
    }
  };
  ...
  const removePhoto = async (user) => {
    ...
    if (!getPhoto(user)) {
      showError("This user does not have a profile photo.");
      return;
    }

    const confirmed = window.confirm(
      `Remove the profile photo for ${getUserName(user)}?`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || "Unable to remove user photo."
        );
      }
      ...
      showSuccess("User photo removed successfully.");
    } catch (err) {
      console.error("REMOVE USER PHOTO ERROR:", err);

      showError(err.message || "Unable to remove user photo.");
    } finally {
      setPhotoLoading("");
    }
  };
  ...
  */
  ...
  const renderAvatar = (user, sizeClass = "") => {
    ...
    return (
      <div className={`users-avatar-placeholder ${sizeClass}`}>
        {getInitials(user)}
      </div>
    );
  };
  ...
  return (
    <div className="users-page">
      <div className="users-page-header">
        <div>
          <div className="users-page-eyebrow">TA-HOSS LOG</div>
          <h1>User Management</h1>
          <p>
            Manage administrators, registration officers, verification officers
            and viewers.
          </p>
        </div>
      ...
      <div className="users-statistics">
        ...
        <div className="users-stat-card">
          ...
          <div>
            ...
            <strong>{statistics.registration}</strong>
          </div>
        </div>
        ...
        <div className="users-stat-card">
          ...
          <div>
            ...
            <strong>{statistics.verification}</strong>
          </div>
        </div>
        ...
        <div className="users-stat-card">
          ...
          <div>
            <span>Viewers</span>
            <strong>{statistics.viewers}</strong>
          </div>
        </div>
      </div>
            ...
            <table className="users-table">
              ...
              <tbody>
                {filteredUsers.map((user) => {
                  ...
                  const isBusy = actionLoading === `${userId}-status`;
                  const isPhotoBusy = photoLoading === userId;
                  ...
                  return (
                    <tr key={userId}>
                      ...
                      {/* ACCOUNT TYPE */}
                      <td>
                        {isSelf ? "Current Account" : "System User"}
                      </td>
        ...
        >
          <div className="users-modal users-form-modal">
            <div className="users-modal-header">
              <div>
                <span>
                  {editingUser ? "USER MANAGEMENT" : "NEW ACCOUNT"}
                </span>
            ...
            <form onSubmit={handleSubmit} className="users-form">
              ...
              <div className="users-form-note">
                ...
                <p>
                  Registration Officers can register residents. Verification
                  Officers can verify records. Viewers have read-only access.
                </p>
              </div>
          ...
          <div className="users-modal users-details-modal">
            ...
            <div className="users-details-content">
              <div className="users-details-profile">
                {renderAvatar(selectedUser, "large")}

                <div>
                  <h3>{getUserName(selectedUser)}</h3>
                  <p>@{selectedUser.username || "N/A"}</p>
                  <span
                    className={`users-status-badge status-${getStatus(
                      selectedUser
                    )}`}
                  >
                    <span />
              ...
              <div className="users-details-grid">
                ...
                <div>
                  <span>Photo</span>
                  <strong>
                    {getPhoto(selectedUser) ? "Uploaded" : "Not uploaded"}
                  </strong>
                </div>

                <div>
                  <span>Account ID</span>
                  <strong>{getUserId(selectedUser) || "N/A"}</strong>
                </div>
              </div>
            </div>
      ...
      {showPasswordModal && selectedUser && (
        <div className="users-modal-overlay">
          <div className="users-modal users-small-modal">
            ...
            <form onSubmit={handlePasswordChange} className="users-form">
              ...
              <div className="users-security-note">
                The Super Admin is changing this user's password
                administratively. The user's current password is not required.
              </div>
      ...
      {showDeleteModal && selectedUser && (
        <div className="users-modal-overlay">
          <div className="users-modal users-small-modal users-danger-modal">
            <div className="users-danger-icon">!</div>

            <h2>Delete User Account?</h2>

            <p>
              You are about to permanently delete the account belonging to{" "}
              <strong>{getUserName(selectedUser)}</strong>.
            </p>

            <p>
              This action should only be performed when the account is no
              longer required.
            </p>
            ...
            <div className="users-modal-footer">
              <button
                ...
                disabled={
                  actionLoading === `${getUserId(selectedUser)}-delete`
                }
              >
                Cancel
              </button>
          ...
          <div className="users-modal users-photo-modal">
            <div className="users-modal-header">
              ...
              <button
                ...
                disabled={photoLoading === getUserId(selectedUser)}
              >
                ×
              </button>
            </div>
            ...
            <div className="users-photo-manager">
              <div className="users-photo-preview">
                {photoPreview ? (
                  <img src={photoPreview} alt="User preview" />
                ) : (
                  <div>{getInitials(selectedUser)}</div>
                )}
              </div>
              ...
              <div className="users-photo-actions">
                ...
                {photoFile && (
                  <button
                    ...
                    disabled={photoLoading === getUserId(selectedUser)}
                  >
                    {photoLoading === getUserId(selectedUser)
                      ? "Uploading..."
                      : "Upload Photo"}
                  </button>
                )}
